 ...
 */
public class Showcase extends WrappingScalableCanvas implements MouseInputListener, MouseWheelListener, Observer, ISelectionManager {
    ...
    public Showcase(Configuration configuration) {
        super(configuration.perception);
        ...
        this.brushIndicatorOverlay = new BrushIndicatorOverlay(configuration, this);
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        ...
        if (e.getSource() == this && configuration.mainFrame.getActiveTool() == PaintCommand.class) {
            // execution of revalidate() in WrappingScalableCanvas.scaleUp()
            // is deferred. but since we are painting an overlay here, we need
            // the new mouseposition now, so we have to lay out manually
            invalidate();
            SwingUtilities.getWindowAncestor(this).validate();
            Point currentMousePosition = getMousePosition();
            if (currentMousePosition != null) {
                brushIndicatorOverlay.setLocation(currentMousePosition);
            }
        }
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            if (configuration.mainFrame.getActiveTool() == PaintCommand.class) {
                painter.setLocation(transformToImageSpace(e.getPoint()));
                paintingTimer.start();
                ...
                if (e.isControlDown())
                    copySelectionCommand.submitPasteKeepingCopy();
                else
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (leftButtonPressed == true) {
            if (activeTool == SelectCommand.class) {
                Point pointInImageSpace = currentMouseDragPosition;
                selectCommand.setLocation(pointInImageSpace);
            } else if (activeTool == PaintCommand.class) {
                painter.setLocation(transformToImageSpace(currentMouseDragPosition));
        ...
        if (activeTool == PaintCommand.class) {
            brushIndicatorOverlay.setLocation(e.getPoint());
        }
    }
